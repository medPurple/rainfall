...
_the main is calling malloc twice and strcpy. m only call puts and n call system_

_we can see a string with the next password_
```
  0x0804845a <+6>:     movl   $0x80485b0,(%esp)
...
_to find the size of the buffer, we will creat a script python that fill the buffer until he crash_
...
```
  ...
  0x73737373 in ?? ()
```
> crash at 0x73737373 so buffer offset is 72
